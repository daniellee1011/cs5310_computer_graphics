...
void CreateGraphicsPipeline() {
  ...
  std::string vertexDebugShaderSource =
      LoadShaderAsString("./shaders/vert.glsl");
  std::string fragmentDebugShaderSource =
      LoadShaderAsString("./shaders/debug_frag.glsl");

  gGraphicsPipelineShaderProgramDebug =
      CreateShaderProgram(vertexDebugShaderSource, fragmentDebugShaderSource);
}
...
void VertexSpecification() {
  ...
  // vertexPositons for the data. For now, this information is just stored in
  // the CPU, and we are going to store this data on the GPU shortly, in a call
  // to glBufferData which will store this information into a vertex buffer
  // object. Note: That I have segregated the data from the OpenGL calls which
  // follow in this function.
  //       It is not strictly necessary, but I find the code is cleaner if
  ...
  // 2 - Vertex
                          -0.0f,  0.5f, -2.5f,  	// Top left vertex
  position
  };
  */
  ...
  // Let's come down here and create another vertex Buffer object(VBO) for our
  // normals.
...
void Draw() {
  ...
  u_ProjectionLocation =
      glGetUniformLocation(gGraphicsPipelineShaderProgramDebug, "u_Projection");
  if (u_ProjectionLocation >= 0) {
    glUniformMatrix4fv(u_ProjectionLocation, 1, GL_FALSE, &perspective[0][0]);
  } else {
    std::cout << "Could not find u_Projection, maybe a mispelling?\n";
