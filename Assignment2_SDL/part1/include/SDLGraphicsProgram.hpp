...
// dependencies.
#if defined(LINUX) || defined(MINGW) || defined(__MINGW64__)
#include <SDL2/SDL.h>
...
public:
  ...
  // They take in functions that are 'void' and take in no arguments(void
  // argument
  void SetPreLoopCallback(
      void (*preLoopfunc)(void)); // Called once before the main loop
  void SetInputCallback(void (*inputFunc)(
      void)); // Handles keyboard, mouse, joystick, etc. events.
  void SetUpdateCallback(void (*updateFunc)(
      void)); // Called every frame and handles updates (e.g. movement)
  void SetRenderCallback(
      void (*renderFunc)(void)); // Called every frame and draws the graphics
...
private:
  ...
  // to use.
  bool
  InitGL(); // Should only be called internally from SDLCreateOpenGLWindow()
...
private:
  ...
  bool m_OpenGLInitialized =
      false; // Flag that tells us if OpenGL was initialized
