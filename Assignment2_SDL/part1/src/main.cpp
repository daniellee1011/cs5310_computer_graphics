...
void PreLoop(void) { std::cout << "Called before the loop one time!\n"; }
...
void Input(void) {
  ...
  while (SDL_PollEvent(&e) != 0) {
    ...
    // Or a video tutorial on keyboard input:
    // https://www.youtube.com/watch?v=piEekEUqvKY
    if (e.type == SDL_KEYDOWN) {
      std::cout << "A key has been pressed" << std::endl;
      switch (e.key.keysym.sym) {
      case SDLK_1:
        std::cout << "1 key has been pressed" << std::endl;
        gSDLGraphicsProgram.SetClearColor(1.0f, 0.0f, 0.0f, 1.0f);
        break;
      case SDLK_2:
        std::cout << "2 key has been pressed" << std::endl;
        gSDLGraphicsProgram.SetClearColor(0.0f, 1.0f, 0.0f, 1.0f);
        break;
      case SDLK_3:
        std::cout << "3 key has been pressed" << std::endl;
