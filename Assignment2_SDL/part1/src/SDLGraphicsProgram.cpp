...
SDLGraphicsProgram::SDLGraphicsProgram(int windowWidth, int windowHeight,
                                       int contextType)
...
void SDLGraphicsProgram::SDLCreateWindow() {
  ...
  if (SDL_Init(SDL_INIT_VIDEO) < 0) {
    ...
    // Create a regular SDL window
    m_window =
        SDL_CreateWindow("Computer Graphics - Shah Section",
  ...
  if (!success) {
    ...
    SDL_Log("SDLGraphicsProgram::SDLGraphicsProgram - No SDL errors detected "
...
void SDLGraphicsProgram::SDLCreateOpenGLWindow() {
  ...
  if (SDL_Init(SDL_INIT_VIDEO) < 0) {
    ...
    m_window = SDL_CreateWindow(
        "Computer Graphics - Shah Section", SDL_WINDOWPOS_UNDEFINED,
...
bool SDLGraphicsProgram::InitGL() {
  ...
  if (m_window == nullptr) {
    errorStream << "InitGL: Error, SDL Window was not previously initialized "
                   "before initializing OpenGL";
  }
  ...
  // If initialization did not work, then print out a list of errors in the
  // constructor. NOTE: Windows users sometimes cannot see the output message
  // for some reason.
  //       Try instead: std::cout << errors.c_str() << std::endl; if there are
  //       errors. or otherwise: std::cout << "SDLGraphicsProgram::InitGL - GLAD
  //       and OpenGL has been initialized\n\n";
  if (!success) {
    errorStream
        << "SDLGraphicsProgram::SDLGraphicsProgram - Failed to initialize!\n";
    std::string errors = errorStream.str();
    ...
    SDL_Log("SDLGraphicsProgram::InitGL - GLAD and OpenGL has been "
            "initialized\n\n");
...
void SDLGraphicsProgram::SetDefaultOpenGLState() {
  if (m_OpenGLInitialized) {
    ...
    glClearColor(m_redChannelBackground, m_greenChannelBackground,
                 m_blueChannelBackground, m_alphaChannelBackground);
...
void SDLGraphicsProgram::TerminateLoop() { m_quitLoop = true; }
...
SDL_Window *SDLGraphicsProgram::GetSDLWindowPointer() { return m_window; }
...
void SDLGraphicsProgram::GetOpenGLVersionInfo() {
  if (m_OpenGLInitialized) {
    ...
    SDL_Log(
        "(Note: Let instructor know if you do not have at least OpenGL 4.3)");
